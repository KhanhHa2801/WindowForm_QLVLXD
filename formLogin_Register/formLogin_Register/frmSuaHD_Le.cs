...
{
    public partial class frmSuaHD_Le : Form
    {
        ...
        private void frmSuaHD_Le_Load(object sender, EventArgs e)
        {
            dtBase.UpdateData("delete from DsHangDaChon where exists(select * from dshangdachon)");

            try
            {
                DataTable tcd = dtBase.DataReader("select tblChiTietHDLe.MaHang, TenHang, GiaBan, SoLuong, ThanhTien from tblChiTietHDLe join tblHang on tblChiTietHDLe.MaHang = tblHang.MaHang where MaHDL = '" + ctrlHDLe.ma_hdd_ + "'");
            ...
            cbxNhanVien.Text = ctrlHDLe.tennv;

            txtSDT.Text = ctrlHDLe.sdt;
            txtNoiGiaoHang.Text = ctrlHDLe.noigiaohang;

            

            try
            {
                DataTable temp = dtBase.DataReader("select NgayDatHang, NoiGiaoHang, TongTien from tblHDLe where MaHDL = '" + ctrlHDLe.ma_hdd_ + "'");

                txtTienHang.Text = temp.Rows[0].ItemArray[2].ToString();
                txtTongTien.Text = temp.Rows[0].ItemArray[2].ToString();
                txtPhaiThu.Text = temp.Rows[0].ItemArray[2].ToString();
            ...
            load_();
            txtTraNgay.Text = txtPhaiThu.Text;
            txtSDT.Text = ctrlHDLe.sdt;
            txtNoiGiaoHang.Text = ctrlHDLe.noigiaohang;
        }
        ...
        private void load_()
        {
            ...
            try
            {
                ...
                txtTongTien.Text = tien_hang.ToString();
        ...
        private int Luu_HD()
        {
            ...
            {
                //To-do: xóa hết các mặt hàng đã có ở Hóa đơn cũ
                dtBase.UpdateData("delete from tblChiTietHDLe where MaHDL = '" + ctrlHDLe.ma_hdd_ + "'");
                dtBase.UpdateData("delete from tblHDLe where MaHDL = '" + ctrlHDLe.ma_hdd_ + "'");
                load_();
                ...
                try
                {
                    dtBase.UpdateData("insert into tblHDLe(MaHDL, MaNV, TenKH, SDT, NgayDat, NoiGiaoHang, TongTien) values('" + ctrlHDLe.ma_hdd_ + "', '" + manv + "', N'" + tenkh + "', '" + txtSDT.Text.Trim() + "', '" + cbxTime.Text + "', N'" + txtNoiGiaoHang.Text.Trim() + "', " + Int32.Parse(txtPhaiThu.Text) + ")");
                }
                catch
                {
                ...
                try
                {
                    for (int i = 0; i < dgvDSHangDaChon.Rows.Count - 1; i++)
                    {
                        ...
                        dtBase.UpdateData("insert into tblChiTietHDLe(MaHDL, MaHang, SoLuong, ThanhTien) values('" + ctrlHDLe.ma_hdd_ + "', '" + mahang + "', '" + soluong + "', '" + thanhtien + "')");
                    }
                }
                catch
                {
        ...
        private void btnLuuVaExport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter;
                ...
                Excel.Range strTongtien = (Excel.Range)exSheet.Cells[i + 10, dgvDSHangDaChon.Columns.Count];
                strTongtien.Font.Size = 12;
                strTongtien.Font.Color = Color.Black;
                strTongtien.Value = "Tổng tiền:";
                ...
                NguoiMua.Value = "Người mua: " + txtKH.Text.Trim();

                Excel.Range sdt1 = (Excel.Range)exSheet.Cells[i + 13, 1];
                string temp1 = "A" + (i + 13).ToString() + ":F" + (i + 13).ToString();
                //MessageBox.Show(temp);
                exSheet.get_Range(temp1).Merge(true);
                sdt1.Font.Size = 12;
                sdt1.Font.Color = Color.Black;
                sdt1.Value = "SDT: " + txtSDT.Text.Trim();
        ...
        {
            if (MessageBox.Show("Bạn có thật sự muốn hủy thay đổi và thoát? ", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == DialogResult.Yes)
            {
