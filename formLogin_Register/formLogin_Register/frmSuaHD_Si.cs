...
{
    public partial class frmSuaHD_Si : Form
    {
        ...
        private void frmSuaHD_Si_Load(object sender, EventArgs e)
        {
            dtBase.UpdateData("delete from DsHangDaChon where exists(select * from dshangdachon)");
            try
            {
                DataTable tcd = dtBase.DataReader("select tblChiTietDatHang.MaHang, TenHang, GiaBan, SoLuong, ThanhTien from tblChiTietDatHang join tblHang on tblChiTietDatHang.MaHang = tblHang.MaHang where MAHDD = '" + ctrlHoaDonSi.ma_hdd_ + "'");
            ...
            cbxKhachHang.Text = ctrlHoaDonSi.tenkh;
            ...
            cbxNhanVien.Text = ctrlHoaDonSi.tennv;

            try
            {
                DataTable temp = dtBase.DataReader("select NgayDatHang, NoiGiaoHang, TongTien, TraNgay, NoLai from tblDatHang where MaHDD = '" + ctrlHoaDonSi.ma_hdd_ + "'");
                ...
                txtTraNgay.Text = temp.Rows[0].ItemArray[3].ToString();
                txtNoLai.Text = temp.Rows[0].ItemArray[4].ToString();

                DataTable kh = dtBase.DataReader("select isnull(TienNo, 0) from tblKhachHang where TenKH = N'" + ctrlHoaDonSi.tenkh + "'");
                try
                {
        ...
        private int Luu_HD()
        {
            ...
            else
            {
                ...
                dtBase.UpdateData("delete from tblChiTietDatHang where MaHDD= '" + ctrlHoaDonSi.ma_hdd_ + "'");
                dtBase.UpdateData("delete from tblDatHang where MaHDD= '" + ctrlHoaDonSi.ma_hdd_ + "'");
                load_();
                ...
                try
                {
                    ...
                    //MessageBox.Show(ttt);
                    dtBase.UpdateData("insert into tblDatHang(MaHDD, MaKH, MaNV, NgayDatHang, NgayGiaoHang, NoiGiaoHang, TongTien, TraNgay, NoLai) values('" + ctrlHoaDonSi.ma_hdd_ + "', '" + makh + "', '" + manv + "', '" + cbxTime.Text + "', '" + cbxTime.Text + "', N'" + diachi.Rows[0].ItemArray[0].ToString() + "', " + Int32.Parse(txtPhaiThu.Text) + ", " + Int32.Parse(txtTraNgay.Text) + ", " + Int32.Parse(txtNoLai.Text) + ")");
                ...
                try
                {
                    for (int i = 0; i < dgvDSHangDaChon.Rows.Count - 1; i++)
                    {
                        ...
                        dtBase.UpdateData("insert into tblChiTietDatHang(MaHDD, MaHang, SoLuong, ThanhTien) values('" + ctrlHoaDonSi.ma_hdd_ + "', '" + mahang + "', '" + soluong + "', '" + thanhtien + "')");
                    }
                }
                catch
                {
        ...
        private void btnLuuVaExport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                exSheet.get_Range("A5:K5").HorizontalAlignment =
